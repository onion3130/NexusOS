...
The current expected migration head is `0017_assistant_grounding`. The migration suite also verifies upgrades from the legacy `0006_v1_hardening` head used by earlier deployments. The readiness check also requires the SQLite FTS5 notes index. Embeddings remain disabled unless an operator configures an approved provider.
