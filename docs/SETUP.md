...
**Status:** v1.7.0 richer document parsing and source expansion (stable) — API health, identity/assistant persistence, session authentication, authenticated web shell, read-only Pi telemetry, bounded assistant gateway, opt-in text SSE streaming, tasks, reminders, notifications, notes/search, source-aware retrieval, optional embeddings, grounded assistant responses with note/source provenance, calendar, finance, media, confirmation-gated host maintenance, verified SQLite backups, encrypted directory replication, confirmation-gated restore, retention cleanup, encryption key rotation, outbound email/push notification channels, audited out-of-process plugins, and audit history are available. External source uploads/imports are available for UTF-8 text, Markdown, and PDF, and single-page HTTPS URL ingestion is available through the Sources workspace. Approved-file synchronization is available with an opt-in 15-minute-to-24-hour polling interval. OCR, crawling, and autonomous memory remain deferred.
