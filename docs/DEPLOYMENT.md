...
**Status:** Hardened Compose/systemd/proxy configuration, bounded Assistant SSE streaming, worker-only PDF/HTML parsing and SSRF-safe URL fetching, encrypted off-host directory replication, confirmation-gated restore, retention cleanup, encryption key rotation, bounded worker recovery, optional semantic retrieval, grounded assistant note/source context and provenance, Maintenance deployment status, and outbound email/push notification channels are implemented. Target-Pi image, TLS trust, restore-drill validation, streaming/fetching through the proxy, and real SMTP/ntfy endpoint checks remain required operator checks.
