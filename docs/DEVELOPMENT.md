...
The task workspace, Notes/Search workspaces, Maintenance workspace, Notifications workspace, and read-only Files/Projects/Git/Docker workspaces are available from authenticated navigation. The notification center polls every 30 seconds and shows outbound email/push delivery state per item. Notes use synchronous SQLite FTS5 projection updates and deterministic retrieval chunks; optional embeddings are generated by the bounded worker and semantic/hybrid search falls back to lexical retrieval when unavailable. The assistant remains usable in disabled-provider mode for conversations; task mutations require a configured provider and explicit approval, while note tools are read-only.
