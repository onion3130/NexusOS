...
External source files use generated server-side filenames. The API never stores or accepts arbitrary client paths. Source versions and chunks are derived from the stored content hash and can be regenerated by the worker.
